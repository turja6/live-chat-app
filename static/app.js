...
activePlugins.forEach(pluginUrl => {
    ...
    script.onerror = () => console.log(`[Plugin Not Found or Failed] ${pluginUrl}`);
...
window.onload = () => {
    ...
    // Trigger onUIReady for plugins
    setTimeout(() => window.ChatHooks.trigger('onUIReady', null), 500);
};

// ... REST OF THE STANDARD LOGIC (modified to trigger hooks) ...
function loadSession() {
    try {
        ...
        if (savedPic) {
            myPicBase64 = savedPic;
            document.getElementById("login-preview").src = savedPic;
        }
...
function connect() {
    ...
    try {
        localStorage.setItem("idly_user", myUsername);
        localStorage.setItem("idly_pic", myPicBase64);
        localStorage.setItem("idly_chat", currentChat);
    } catch(e) {}
    ...
    ws.onmessage = (event) => {
        try {
            ...
            // LET PLUGINS INTERCEPT/MODIFY DATA
            data = window.ChatHooks.trigger('onMessageRender', data);
            if (!data) return; // If a plugin completely swallowed the event, stop here.
...
function attemptReconnect() {
    ...
    reconnectAttempts++;
    updateConnectionStatus(false);
    showToast(`Reconnecting in ${delay/1000}s...`, "error");
...
function renderHistory(data) {
    ...
    if (!data.data || data.data.length === 0) {
        msgs.innerHTML = `<div class="welcome-message"><h3>Welcome to ${currentChat === 'Public' ? 'Public Lobby' : currentChat}!</h3><p>No messages yet. Be the first to say hello! 👋</p></div>`;
    } else {
        data.data.forEach(m => drawMessage(m, false));
        scrollToBottom();
    }
}

function handleIncomingChat(data) {
    const isPublic = data.receiver === "Public";
    const isForMe = data.receiver === myUsername;
    const isFromMe = data.sender === myUsername;
    let show = false;
    
    if (currentChat === "Public" && isPublic) show = true;
    else if (currentChat !== "Public") {
        if (isFromMe && data.receiver === currentChat) show = true;
        if (data.sender === currentChat && isForMe) show = true;
    }

    if (show) { drawMessage(data, true); scrollToBottom(); } 
    else if (!isFromMe) { showToast(`New message from ${isPublic ? "Public Lobby" : data.sender}`, "success"); }
}

function drawMessage(msg, isNew = true) {
    const msgs = document.getElementById("messages");
    const welcomeEl = msgs.querySelector('.welcome-message');
    if (welcomeEl) welcomeEl.remove();

    const div = document.createElement("div");
    div.className = "message-group";
    const isMe = msg.sender === myUsername;
    if (isMe) div.classList.add("own-message");
    ...
    let contentHtml = "";
    if (isImage) {
        contentHtml = `<img src="${msg.content}" class="msg-image" onclick="openLightbox('${msg.content.replace(/'/g, "\\'")}')">`;
    } else if (isFile) {
        const fileName = msg.fileName || 'document.txt';
        contentHtml = `<a href="${msg.content}" download="${escapeHtml(fileName)}" class="file-bubble" onclick="event.stopPropagation()"><div class="file-icon">📄</div><div class="file-info"><div class="file-name">${escapeHtml(fileName)}</div><div class="file-download-text">Click to download</div></div></a>`;
    } else {
...
function sendMsg() {
    ...
    if (ws && ws.readyState === WebSocket.OPEN) {
        ...
        if (replyingTo) {
            msgObj.reply_to_id = replyingTo.id;
            msgObj.reply_to_name = replyingTo.sender;
            msgObj.reply_to_text = replyingTo.content;
            replyingTo = null;
            document.getElementById('reply-preview').classList.remove('active');
        }

        // LET PLUGINS MODIFY MESSAGE BEFORE SENDING
...
function handleTyping() {
    ...
    clearTimeout(typingTimeout);
    typingTimeout = setTimeout(() => { isTyping = false; }, 1500);
}
...
function handleTypingIndicator(data) {
    ...
    indicatorTimeout = setTimeout(() => { currentlyTypingUsers.clear(); updateTypingUI(); }, 3000);
}
...
function showMessageContext(e, id, content, sender) {
    ...
    contextMenuTarget = { id, content, sender };
    const ctx = document.getElementById('context-menu');
    const deleteBtn = document.getElementById('ctx-delete-btn');
    if (sender === myUsername) deleteBtn.style.display = 'flex';
    ...
    if (contextMenuTarget && contextMenuTarget.sender === myUsername) {
        ...
        if (msgEl) {
            ...
            if (group) {
                ...
                messageStore = messageStore.filter(m => m.id !== contextMenuTarget.id);
            }
        }
...
function scrollToMessage(msgId) {
    ...
    if (el) {
        ...
        if (group) { group.style.background = 'rgba(99, 102, 241, 0.15)'; setTimeout(() => { group.style.background = ''; }, 1500); }
    }
}
...
function updateFooterProfile() {
    const avatarEl = document.getElementById("my-avatar-footer");
    const nameEl = document.getElementById("my-username-footer");
    if (avatarEl) avatarEl.src = myPicBase64;
...
function filterUsers() {
    ...
    items.forEach(item => {
        ...
        if (name) item.style.display = name.innerText.toLowerCase().includes(query) ? 'flex' : 'none';
    });
}
...
function loadTheme() {
    const theme = localStorage.getItem('idly_theme') || 'dark';
...
function toggleTheme() {
    ...
    document.body.setAttribute('data-theme', theme);
    localStorage.setItem('idly_theme', theme);
}

function clearData() {
    if (confirm("Clear all local settings and data? You will return to login.")) {
...
function showToast(message, type = "") {
    ...
    toast.className = 'toast'; if (type) toast.classList.add(type);
