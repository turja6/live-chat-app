...
def get_chat_history(user1, user2):
    ...
    try:
        if user2 == "Public":
            msgs = db.query(Message).filter(Message.receiver == "Public").order_by(Message.id.desc()).limit(100).all()
        else:
        ...
        return [{"sender": m.sender, "profile_pic": m.profile_pic, "content": m.content, "timestamp": m.timestamp} for m in reversed(msgs)]
    finally:
        db.close()